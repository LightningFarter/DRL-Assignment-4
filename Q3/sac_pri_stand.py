...
class PrioritizedReplayBuffer:
    def __init__(self, max_size=int(1e6), alpha=0.6, epsilon=1e-6):
        self.max_size = max_size
        self.alpha = alpha
    ...
    def __len__(self):
        return len(self.buffer) if not self.full else self.max_size
...
class SACAgent:
    def __init__(self, state_dim: int, action_dim: int, total_episodes=MAX_EPISODE) -> None:
        ...
        self.beta_schedule = lambda x: min(1.0, BETA + x * (1.0 - BETA) / BUFFER_SIZE)
    ...
    def act(self, state: np.ndarray) -> np.ndarray:
        with torch.no_grad():
            ...
            action_tensor, _ = self.actor.sample(state_tensor.unsqueeze(0))
    ...
    def update(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> None:
        indices, (states, actions, rewards, next_states, dones), weights = batch
        ...
        td_errors = 0.5 * (F.mse_loss(current_q1, target_q, reduction='none') + F.mse_loss(current_q2, target_q, reduction='none'))
    ...
    def save_checkpoint(self, checkpoint_dir: str, episode: int) -> None:
        ...
        torch.save(checkpoint, os.path.join(checkpoint_dir, f'sac_checkpoint_{episode}.pth'))
...
def train(resume_checkpoint=LOAD_MODEL_PATH):
    ...
    for episode in range(start_episode, max_episodes):
        ...
        print(f"Episode {episode} | Reward: {episode_reward:.1f} | Time: {episode_time:.1f}s")
