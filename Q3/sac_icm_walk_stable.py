...
# --- Hyperparameters ---
ENV_NAME = "humanoid-walk" # Changed to walk for a slightly more dynamic task than stand
SEED = 1
...
class SACAgent:
    ...
    def save_checkpoint(self, checkpoint_dir: str, timestep: int) -> None:
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f'sac_icm_ckpt_walk_{timestep}.pth')
        checkpoint = {
            'timestep': timestep,
    ...
    def load_checkpoint(self, checkpoint_path: str) -> int:
        if not os.path.exists(checkpoint_path):
             ...
             return 0
        checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False) # Ensure loading to correct device
